...
create(HostId,VmId,VmDir,Cookie)->
    Result=case rpc:call(sd:dbase_node(),db_server,read,[HostId]) of
	       []->
...
create_vm(HostId,VmId,VmDir,Cookie)->
    Vm=list_to_atom(VmId++"@"++HostId),
    true=stop_vm(Vm),  
    Reply=case os:cmd("erl -sname "++VmId++" -setcookie "++Cookie++" -detached") of
	      ok->
